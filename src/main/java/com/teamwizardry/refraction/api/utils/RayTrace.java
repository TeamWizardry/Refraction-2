...
public class RayTrace {
	...
	private RayTraceResult traceBlock(@Nonnull Vec3d start, @Nonnull Vec3d end) {
		RayTraceResult defaultResult = null;
		...
		while (k1-- >= 0) {
			if (l == i && i1 == j && j1 == k) {
				return returnLastUncollidableBlock ? defaultResult : null;
			}
			...
			if (!skipBlockList.contains(targetPos)) {
				if (!ignoreBlocksWithoutBoundingBoxes || targetState.getMaterial() == Material.PORTAL || targetState.getCollisionBoundingBox(world, targetPos) != Block.NULL_AABB) {
					if (targetBlock.canCollideCheck(targetState, false)) {
						RayTraceResult blockTrace;
						if (targetBlock instanceof IBlockCollisionRayTrace)
							blockTrace = ((IBlockCollisionRayTrace) targetBlock).rayCollision(targetState, world, targetPos, start, end);
						else
							blockTrace = targetState.collisionRayTrace(world, targetPos, start, end);

						if (blockTrace != null) {
							return blockTrace;
						}
					} else {
						defaultResult = new RayTraceResult(RayTraceResult.Type.MISS, start, enumfacing, targetPos);
					}
				}
			}
		}
